<?php
/**
 * submissions-delete-all.php
 *
 * Deletes all submissions for a study after verifying user ownership.
 ...
 * @param int|null $deletedCount Optional count of deleted submissions
...
function verifyStudyOwnership($database, string $studyCode, int $userID)
{
    try {
        ...
        if ($studyOwnerID !== $userID) {
            error_log("Authorization violation: User $userID attempted to delete submissions for study $studyID owned by user $studyOwnerID");
            return false;
        }
...
/**
 * Delete all submissions for a study
 *
 * @param object $database Medoo database instance
 * @param int $studyID Study ID
 * @return array Result with 'success' boolean, 'deletedCount' int, and 'message' string
 */
function deleteAllSubmissions($database, int $studyID): array
{
    $result = ['success' => false, 'deletedCount' => 0, 'message' => ''];
    
    try {
        // Count submissions before deletion
        $submissionCount = $database->count("submissions", [
            "studyID" => $studyID
        ]);
        
        if ($submissionCount === 0) {
            $result['success'] = true;
            $result['message'] = 'No submissions to delete';
            return $result;
        }
        
        // Delete all submissions for the study
        $deleteResult = $database->delete("submissions", [
            "studyID" => $studyID
        ]);
        
        if ($deleteResult === false) {
            error_log("Failed to delete submissions for study $studyID");
            $result['message'] = 'Failed to delete submissions';
            return $result;
        }
        
        // PDOStatement::rowCount() returns the number of affected rows
        $deletedCount = is_object($deleteResult) ? $deleteResult->rowCount() : $submissionCount;
        
        $result['success'] = true;
        $result['deletedCount'] = $deletedCount;
        $result['message'] = "Successfully deleted $deletedCount submissions";
        
        error_log("Study $studyID: Deleted $deletedCount submissions");
        
        return $result;
        
    } catch (Exception $e) {
        error_log("Error deleting submissions for study $studyID: " . $e->getMessage());
        $result['message'] = 'An error occurred while deleting submissions';
        return $result;
    }
}
...
if (isset($_POST['csrf_token']) && isset($_SESSION['csrf_token'])) {
    if ($_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        error_log("CSRF token validation failed in submission deletion request");
        sendResponse('bad', 'Invalid request', null, null, 403);
    }
} else {
    // Log warning but allow request (backward compatibility)
    error_log("WARNING: Submission deletion request without CSRF token - update frontend to include token");
}
...
// Delete all submissions for the study
$deleteResult = deleteAllSubmissions($database, $studyID);
